...
public class BadDCSockTakeDAO {
    private ZHOKIGSQLiteOpenHelper helper;
    private SQLiteDatabase db;
    private Context mContext;

    public BadDCSockTakeDAO(Context c) {
    ...
    public void add(DCStockTake model) {
        ...
        db.beginTransaction();
        try {
            db.execSQL("insert into tb_bad_stock_take (prod_id,qty,oper_name,status) values(?,?,?,?)", new Object[] { model.getProd_id() ,model.getQty(), model.getOper_name(), model.getStatus()});
    ...
    public void clearTable() {
        db = helper.getWritableDatabase();
        db.execSQL("delete from tb_bad_stock_take");
    }
    ...
    public long getCount() {
        ...
        Cursor cursor = db.rawQuery("select count(_id) from tb_bad_stock_take", null);
    ...
    public List<DCStockTake> search() {
        ...
        Cursor cursor=db.rawQuery("select * from tb_bad_stock_take", null);
    ...
    public void updataStatus(String prod_id, int status) {
        ...
        try {
            ...
            db.update("tb_bad_stock_take", cv, "prod_id = ?", new String[]{prod_id});
            db.setTransactionSuccessful();
        } finally {
            db.endTransaction();
        }
    ...
    public void deleteByProdId(String prod_id) {
        ...
        db.beginTransaction();
        try {
            db.delete("tb_bad_stock_take", "prod_id = ?", new String[]{prod_id});
            db.setTransactionSuccessful();
        } finally {
            db.endTransaction();
        }
